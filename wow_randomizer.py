...
def export_loot_addon_data(
    db: DB,
    output_path: str,
    max_items_per_creature: int,
) -> None:
    if max_items_per_creature < 1:
        raise RuntimeError("--max-items-per-creature must be >= 1")

    if not db.table_exists(TABLE_CREATURE_LOOT):
        raise RuntimeError("Table `creature_loot_template` not found in world DB")

    has_creature_template = db.table_exists(TABLE_CREATURE_TEMPLATE)
    has_item_template = db.table_exists(TABLE_ITEM)

    creature_name_expr = "ct.name AS creature_name" if has_creature_template else "'' AS creature_name"
    item_name_expr = "it.name AS item_name" if has_item_template else "'' AS item_name"
    creature_join = (
        f"LEFT JOIN `{TABLE_CREATURE_TEMPLATE}` ct ON ct.entry = clt.Entry"
        if has_creature_template
        else ""
    )
    item_join = (
        f"LEFT JOIN `{TABLE_ITEM}` it ON it.entry = clt.Item"
    ...
    rows = db.fetchall(
        ...
        "clt.Item AS item_entry, "
        "clt.ChanceOrQuestChance AS chance, "
        "clt.MinCountOrRef AS min_count, "
        "clt.MaxCount AS max_count, "
        "clt.GroupId AS group_id, "
        f"{creature_name_expr}, "
        f"{item_name_expr} "
        f"FROM `{TABLE_CREATURE_LOOT}` clt "
        f"{creature_join} "
        f"{item_join} "
        "WHERE clt.Item > 0 "
        "ORDER BY clt.Entry ASC, ABS(clt.ChanceOrQuestChance) DESC, clt.Item ASC"
    )

    loot_by_creature: Dict[int, List[Dict[str, Any]]] = defaultdict(list)
    creature_names: Dict[int, str] = {}
    item_names: Dict[int, str] = {}

    for row in rows:
        creature_entry = int(row["creature_entry"])
        item_entry = int(row["item_entry"])
        chance_raw = float(row["chance"] or 0)
        min_count = int(row["min_count"] or 1)
        max_count = int(row["max_count"] or min_count)
        ...
        loot_by_creature[creature_entry].append(
            {
                "item": item_entry,
                "chance": abs(chance_raw),
                "min": min_count,
                "max": max_count,
                "group": group_id,
                "quest": chance_raw < 0,
        ...
        creature_name = str(row.get("creature_name") or "").strip()
        item_name = str(row.get("item_name") or "").strip()
        if creature_name:
            creature_names[creature_entry] = creature_name
        if item_name:
            item_names[item_entry] = item_name

    total_entries = 0
    truncated_creatures = 0
    for creature_entry in sorted(loot_by_creature):
        loot_rows = loot_by_creature[creature_entry]
        loot_rows.sort(key=lambda x: (-float(x["chance"]), int(x["item"])))
        if len(loot_rows) > max_items_per_creature:
            loot_by_creature[creature_entry] = loot_rows[:max_items_per_creature]
            truncated_creatures += 1
        total_entries += len(loot_by_creature[creature_entry])

    output = resolve_output_path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

    with open(output, "w", encoding="utf-8", newline="\n") as f:
        f.write("-- Auto-generated by wow_randomizer.py export-addon\n")
        f.write("-- Do not edit manually.\n\n")

        f.write("WRL_LootByCreature = {\n")
        for creature_entry in sorted(loot_by_creature):
            f.write(f"  [{creature_entry}] = {{\n")
            for loot in loot_by_creature[creature_entry]:
                quest_flag = "true" if bool(loot["quest"]) else "false"
                f.write(
                    "    { item = %d, chance = %.4f, min = %d, max = %d, group = %d, quest = %s },\n"
                    % (
                        int(loot["item"]),
                        float(loot["chance"]),
                        int(loot["min"]),
        ...
        f.write("WRL_CreatureNames = {\n")
        for creature_entry in sorted(creature_names):
            name = lua_escape(creature_names[creature_entry])
            f.write(f'  [{creature_entry}] = "{name}",\n')
        f.write("}\n\n")

        f.write("WRL_ItemNames = {\n")
        for item_entry in sorted(item_names):
            name = lua_escape(item_names[item_entry])
            f.write(f'  [{item_entry}] = "{name}",\n')
        f.write("}\n\n")

        f.write("WRL_Metadata = {\n")
        f.write(f"  generated_at = {int(time.time())},\n")
        f.write(f"  creatures = {len(loot_by_creature)},\n")
        f.write(f"  entries = {total_entries},\n")
        f.write("}\n")

    print("Addon data export completed.")
    print(f"  output: {output}")
    print(f"  creatures: {len(loot_by_creature)}")
    print(f"  loot rows: {total_entries}")
    print(f"  max_items_per_creature: {max_items_per_creature}")
    if truncated_creatures > 0:
        print(f"  truncated creatures: {truncated_creatures}")
    if not has_creature_template:
        print("  warning: creature_template not found, creature names were omitted")
    if not has_item_template:
        print("  warning: item_template not found, item names were omitted")
...
def parse_args() -> argparse.Namespace:
    ...
    reroll_parser.add_argument(
        "--export-addon-output",
        default=None,
        help="Optional Lua file path for addon export after reroll.",
    )
    reroll_parser.add_argument(
        "--export-max-items-per-creature",
        type=int,
        default=25,
        help="Max loot entries per creature in addon export.",
    )
    randomize_parser.add_argument(
        "--export-addon-output",
        default=None,
        help="Optional Lua file path for addon export after randomize.",
    )
    randomize_parser.add_argument(
        "--export-max-items-per-creature",
        type=int,
    ...
    export_parser = sub.add_parser(
        "export-addon",
        help="Export current DB loot into addon Lua data file",
    )
    export_parser.add_argument(
        "--output",
        default="addon/WowRandomizerLoot/WowRandomizerLootData.lua",
        help="Output Lua file path",
    )
    export_parser.add_argument(
        "--max-items-per-creature",
        type=int,
        default=25,
        help="Max loot entries per creature in exported addon data.",
    )
...
def main() -> int:
    ...
    try:
        if args.command == "init-backup":
            ...
            if getattr(args, "export_addon_output", None):
                export_loot_addon_data(
                    db,
                    args.export_addon_output,
                    int(args.export_max_items_per_creature),
                )
        elif args.command == "status":
            show_status(db)
        elif args.command == "export-addon":
            export_loot_addon_data(
                db,
                args.output,
                int(args.max_items_per_creature),
            )
        else:
