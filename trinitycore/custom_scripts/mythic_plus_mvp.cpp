...
namespace
{
    ...
    // Visual effect on portal (WotLK 3.3.5: 32264=portal shimmer, 61722=Nature Portal State)
    constexpr uint32 MPLUS_PORTAL_VISUAL_SPELL = 32264;
    ...
    private:
        ...
        void DespawnActivePortal(bool announce)
        {
            if (_activePortal.active)
            {
                if (Map* map = MapManager::instance()->CreateBaseMap(_activePortal.mapId))
                {
                    ...
                    for (ObjectGuid const& guid : _activePortal.waveCreatureGuids)
        ...
        void ApplyPortalVisualEffect(Creature* gate) const
        {
            if (!gate)
                return;
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(MPLUS_PORTAL_VISUAL_SPELL))
                gate->CastSpell(gate, MPLUS_PORTAL_VISUAL_SPELL, true);
        }
        ...
        void SpawnWaveCreatures(std::time_t now)
        {
            ...
            for (uint32 i = 0; MPLUS_WAVE_CREATURE_ENTRIES[i] != 0; ++i)
            {
                if (sObjectMgr->GetCreatureTemplate(MPLUS_WAVE_CREATURE_ENTRIES[i]))
                    validEntries.push_back(MPLUS_WAVE_CREATURE_ENTRIES[i]);
            }
            if (validEntries.empty())
                return;

            uint32 secondsLeft = uint32(_activePortal.expiresUnix - now);
            if (secondsLeft < 1)
                secondsLeft = 1;
            uint32 despawnMs = secondsLeft * 1000u;

            uint8 count = uint8(urand(MPLUS_WAVE_MIN_COUNT, MPLUS_WAVE_MAX_COUNT));
            float const twoPi = 6.28318530718f;

            for (uint8 i = 0; i < count; ++i)
            {
                float angle = frand(0.0f, twoPi);
                float dist = frand(5.0f, MPLUS_WAVE_SPAWN_RADIUS);
                float wx = _activePortal.pos.GetPositionX() + dist * std::cos(angle);
                float wy = _activePortal.pos.GetPositionY() + dist * std::sin(angle);
                float wz = _activePortal.pos.GetPositionZ();

                map->LoadGrid(wx, wy);

                Position spawnPos;
                spawnPos.Relocate(wx, wy, wz, 0.0f);

                uint32 entry = validEntries[urand(0u, uint32(validEntries.size() - 1))];
                if (Creature* c = map->SummonCreature(entry, spawnPos, nullptr, despawnMs))
                {
                    c->SetReactState(REACT_AGGRESSIVE);
                    _activePortal.waveCreatureGuids.push_back(c->GetGUID());
                }
            }
        }
        ...
        void SpawnPortalRadiusMarkers(Map* map, ActivePortal& portal, uint32 despawnMs) const
        {
            ...
            for (uint8 i = 0; i < MPLUS_PORTAL_MARKER_POINTS; ++i)
            {
                ...
                float mx = portal.pos.GetPositionX() + MPLUS_PORTAL_BONUS_RADIUS * std::cos(angle);
                float my = portal.pos.GetPositionY() + MPLUS_PORTAL_BONUS_RADIUS * std::sin(angle);
                float mz = portal.pos.GetPositionZ() + MPLUS_PORTAL_MARKER_Z_OFFSET;
                map->LoadGrid(mx, my);

                Position markerPos;
                markerPos.Relocate(mx, my, mz, angle);
        ...
        void SpawnRandomPortal(std::time_t now)
        {
            ...
            ApplyPortalVisualEffect(summonedGate);
        ...
        void HandleWorldPortalTick(uint32 diff)
        {
            std::time_t now = std::time(nullptr);
